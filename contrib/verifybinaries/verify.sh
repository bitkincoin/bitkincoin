#!/usr/bin/env bash
# Copyright (c) 2016 The Bitkincoin Core developers
# Distributed under the MIT software license, see the accompanying
# file COPYING or http://www.opensource.org/licenses/mit-license.php.

###   This script attempts to download the signature file SHA256SUMS.asc from
###   bitkincoincore.org and bitkincoin.org and compares them.
...
WORKINGDIR="/tmp/bitkincoin_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://bitkincoincore.org"
HOST2="https://bitkincoin.org"
BASEDIR="/bin/"
VERSIONPREFIX="bitkincoin-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'bitkincoin-',
...
if [ $? -ne 0 ]; then
   echo "bitkincoin.org failed to provide signature file, but bitkincoincore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "bitkincoin.org and bitkincoincore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Bitkincoin Core binary release signing key installed?"
   fi
