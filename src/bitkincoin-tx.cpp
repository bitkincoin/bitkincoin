...
static void SetupBitkincoinTxArgs()
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " bitkincoin-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  bitkincoin-tx [options] <hex-tx> [commands]  Update hex-encoded bitkincoin transaction\n" +
            "or:     bitkincoin-tx [options] -create [commands]   Create hex-encoded bitkincoin transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded bitkincoin transaction
