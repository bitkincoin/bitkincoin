...
const char * const BITCOIN_CONF_FILENAME = "bitkincoin.conf";
const char * const BITCOIN_PID_FILENAME = "bitkincoind.pid";
...
public:
    ...
    static inline std::pair<bool,std::string> GetArg(const ArgsManager &am, const std::string& arg)
    {
        ...
        // argument value seen from the command line (so "bitkincoind -foo=bar
...
#else
    const char* pszModule = "bitkincoin";
#endif
    if (pex)
        return strprintf(
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\Bitkincoin
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\Bitkincoin
    // Mac: ~/Library/Application Support/Bitkincoin
    // Unix: ~/.bitkincoin
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Bitkincoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Bitkincoin";
#else
    // Unix
    return pathRet / ".bitkincoin";
#endif
#endif
}
...
std::string CopyrightHolders(const std::string& strPrefix)
{
    ...
    // Check for untranslated substitution to make sure Bitkincoin Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Bitkincoin Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + "The Bitkincoin Core developers";
    }
