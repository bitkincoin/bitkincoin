 ...
 * between bitkincoind, and bitkincoin-qt and non-server tools.
*/
...
void Shutdown()
{
    ...
    RenameThread("bitkincoin-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/bitkincoin/bitkincoin>";
    const std::string URL_WEBSITE = "<https://bitkincoincore.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("bitkincoin-loadblk");
...
/** Sanity checks
 *  Ensure that Bitkincoin is running in a usable environment with all
...
{
    // Make sure only a single Bitkincoin process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if bitkincoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if bitkincoin is started while in a temporary directory.\n",
